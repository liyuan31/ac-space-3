...
%     fprintf(ini.stimfilepointer,'\n');
    ...
    for instructions = 1:ninstr;
        instrpic=imread(strcat(instrfolder,'/StartInstr',num2str(instructions),'.jpg'));
        instrtex = Screen('MakeTexture',w,instrpic);
        Screen('DrawTexture',w,instrtex,[],InstrRect);
%         line1 = 'Hello';
%         line2 = '\n This is the demo version instruction page';
...
%             'center', screen_y_pixels * 0.25, [255 255 255]);
    ...
    for block=0:nblocks                 % Block 0: Practice block, Block 1:nblocks: Experimental blocks
        ...
        for trial = 1:trialno  % 18*2*2=72 trialno
            ...
            % 20190820 added: record stimuli for each trial
            fprintf(ini.stimfilepointer,'%s\n', 'BEGIN');
            fprintf(ini.stimfilepointer,'%d,%d,%d,%d,%d,%d,%f,%d,%d\n', current_trial_cond.');
            fprintf(ini.stimfilepointer,'%f,%f,%f,%f,%f,%f,%d,%d\n', trial_stim.');
            fprintf(ini.stimfilepointer,'%s\n', 'END');
            ...
            % Draw items
            Screen('FillRect', w, [97 97 97]',trial_stim(:,1:4)');    % Draw the squares
            for i = 1:size(trial_stim,1)    % Draw the digits on the squares
                Screen('DrawText', w, num2str(trial_stim(i,8)), ...
                    trial_stim(i,5)-5.5, trial_stim(i,6)-11.5, [255 255 255]'); % Position of text adjusted a little to centre in each square
            end
